...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action == 'update_profil') {
        ...
        if ($update) {
            ...
            setFlash('Gagal memperbarui profil', 'danger');
        }
    }
    ...
    if ($action == 'update_password') {
        ...
        if (!password_verify($password_lama, $user['password'])) {
            setFlash('Password lama tidak sesuai', 'danger');
            ...
            $update = query("UPDATE users SET password = ? WHERE id = ?", [$password_hash, $current_user['id']], 'si');
            ...
            if ($update) {
                ...
                setFlash('Gagal mengubah password', 'danger');
            }
        }
    }
    ...
    if ($action == 'upload_foto') {
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
            ...
            if (!in_array($ext, $allowed)) {
                setFlash('Format file harus JPG, JPEG, atau PNG', 'danger');
            } else {
                $newname = 'foto_' . $current_user['id'] . '_' . time() . '.' . $ext;
                $upload_path = '../uploads/users/' . $newname;
                ...
                if (move_uploaded_file($_FILES['foto']['tmp_name'], $upload_path)) {
                    ...
                    $update = query("UPDATE users SET foto = ? WHERE id = ?", 
                                  ['uploads/users/' . $newname, $current_user['id']], 'si');
                    
                    if ($update) {
                        setFlash('Foto profil berhasil diupdate', 'success');
                        header('Location: profil.php');
                        exit;
                    }
                }
            ...
            setFlash('Pilih file foto terlebih dahulu', 'warning');
        }
    }
}
...
// Statistik siswa
$stats = [
    'eskul' => query("SELECT COUNT(*) as total FROM anggota_ekskul WHERE user_id = ? AND status = 'diterima'", 
                     [$current_user['id']], 'i')->fetch_assoc()['total'],
    'prestasi' => query("SELECT COUNT(*) as total FROM prestasis p JOIN anggota_ekskul ae ON p.anggota_id = ae.id WHERE ae.user_id = ?", 
                        [$current_user['id']], 'i')->fetch_assoc()['total'],
    'presensi' => query("SELECT COUNT(*) as total FROM presensis p JOIN anggota_ekskul ae ON p.anggota_id = ae.id WHERE ae.user_id = ?", 
                        [$current_user['id']], 'i')->fetch_assoc()['total']
];
?>
<!DOCTYPE html>
            ...
            <div class="col-md-2 bg-light p-0" style="min-height: calc(100vh - 56px);">
                <div class="sidebar">
                    <nav class="nav flex-column">
                        ...
                        <a class="nav-link active" href="<?php echo BASE_URL; ?>siswa/profil.php">
                        ...
                        <div class="card border-0 shadow-sm text-center">
                            <div class="card-body py-4">
                                ...
                                <img src="<?php echo BASE_URL . $current_user['foto']; ?>" 
                                     ...
                                     alt="Foto Profil">
                                <?php else: ?>
                                <div class="rounded-circle profile-img mb-3 mx-auto bg-primary text-white d-flex align-items-center justify-content-center" style="font-size: 5rem;">
                                ...
                                <div class="text-start">
                                    <p class="mb-2"><i class="bi bi-person-badge text-primary"></i> <strong>NIS:</strong> <?php echo $current_user['nis']; ?></p>
                                    <p class="mb-2"><i class="bi bi-envelope text-primary"></i> <strong>Email:</strong> <?php echo $current_user['email']; ?></p>
                                    <p class="mb-2"><i class="bi bi-telephone text-primary"></i> <strong>Telepon:</strong> <?php echo $current_user['no_hp'] ?: '-'; ?></p>
                                    <p class="mb-0"><i class="bi bi-book text-primary"></i> <strong>Kelas:</strong> <?php echo $current_user['kelas']; ?></p>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="text" class="form-control" 
                                                   value="<?php echo $current_user['nis']; ?>" disabled>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label class="form-label">Kelas</label>
                                            <input type="text" class="form-control" 
                                                   value="<?php echo $current_user['kelas']; ?>" disabled>
                                        </div>
                                        <div class="col-md-6 mb-3">
